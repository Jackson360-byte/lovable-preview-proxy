...
function buildRequestHeaders(req, target) {
  ...
  // Tell upstream (WordPress, etc.) it's behind an HTTPS reverse proxy so
  // is_ssl() returns true and admin URLs/redirects use https. Without these,
  // WP can clear the secure auth cookie when navigating wp-admin.
  out["x-forwarded-proto"] = "https";
  out["x-forwarded-host"] = target.domain;
  out["x-forwarded-for"] = req.ip || "127.0.0.1";
...
function rewriteHtml(html, target) {
  ...
  // Rewrite root-relative WordPress URLs in attributes AND inline JS strings
  // so AJAX (admin-ajax.php), form actions, and dynamic asset loads hit /p/...
  // directly instead of bouncing through the 307 fallback (which some XHR
  // clients drop credentials/bodies on).
  // Matches: "/wp-admin/...", '/wp-content/...', '/wp-includes/...', '/wp-json/...',
  //          '/wp-login.php', '/xmlrpc.php' inside any quote context.
  out = out.replace(/(["'`=(,\s])\/(wp-admin|wp-content|wp-includes|wp-json|wp-login\.php|wp-cron\.php|xmlrpc\.php)/g,
    (_m, pre, p) => `${pre}/p/${p}`);
  // Inject <base href="/p/"> so plain relative URLs resolve via the proxy.
  if (/<head[^>]*>/i.test(out)) {
...
function rewriteCss(css, target) {
  const absRe = new RegExp(`(https?:)?\/\/${target.domain.replace(/\./g, "\\.")}`, "gi");
  let out = css.replace(absRe, "");
  // Rewrite root-relative wp-* asset URLs inside url(...) too.
  out = out.replace(/url\(\s*(["']?)\/(wp-admin|wp-content|wp-includes)/g,
    (_m, q, p) => `url(${q}/p/${p}`);
  return out;
}
...
// Fallback: any other path with a session cookie (e.g. form posts to /wp-login.php
// or assets requested at root) gets re-routed through the proxy at /p<path>.
// Preserves method, headers, body via 307.
app.use((req, res, next) => {
  if (req.path === "/" || req.path.startsWith("/start") || req.path.startsWith("/p")) return next();
  if (!req.cookies[SESSION_COOKIE]) return next();
  const dest = "/p" + req.originalUrl;
